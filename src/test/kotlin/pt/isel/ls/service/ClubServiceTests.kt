...
class ClubServiceTests {
    ...
    fun `create club with duplicate name should fail`() {
        val ownerResult = userService.createUser(Name("Ric"), Email("ric@email.com"), createPassword("a"))
        assertTrue(ownerResult.isSuccess)
    ...
    fun `find club by identifier`() {
        val owner = userService.createUser(Name("Ric1"), Email("ric@email.com"), createPassword("a"))
