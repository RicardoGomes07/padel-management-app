...
class ClubWebApiTests {
    ...
    private val user = Request(Method.POST, "http://localhost:8080/users")
    ...
    fun `club creation with valid Name and User`() {
        ...
        val club = Request(Method.POST, "http://localhost:8080/clubs")
            .header("Content-Type", "application/json")
    ...
    fun `get club info with valid club id`() {
        ...
        val token = response.bodyString().split("token")[1].split("\"")[2]
        val clubId = 1
        val getClubInfoRequest = Request(Method.GET, "http://localhost:8080/clubs/$clubId")
            .header("Authorization", token)
    ...
    fun `get club info with invalid club id`() {
        ...
        val getClubInfoRequest = Request(Method.GET, "http://localhost:8080/clubs/$invalidClubId")
            .header("Authorization", token)
        val getClubInfoResponse = client(getClubInfoRequest)
    ...
    fun `get all clubs with valid token`() {
        val response = client(user)
        val token = response.bodyString().split("token")[1].split("\"")[2]
        val getAllClubsRequest = Request(Method.GET, "http://localhost:8080/clubs")
            .header("Authorization", token)
    ...
    fun `get available hours with invalid date`() {
        ...
        val getAvailableHoursRequest =
            Request(Method.GET, "http://localhost:8080/clubs/$clubId/courts/$courtId/available?date=$date")
                .header("Authorization", token)
                .query("date", date)
